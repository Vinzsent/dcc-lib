...
@php
    ...
    // DCC BED (all) admins get to choose; single sub-campus admins are locked to their own
    $bedCanChoose = session('location') === 'DCC BED';
@endphp
...
@section('content')
    ...
    <div class="card bg-white p-6 rounded-lg shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <h3 class="text-gray-700 font-bold text-lg">BED Shelves — Basic Education</h3>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    @forelse ($shelves as $shelf)
                        <tr class="border-b border-gray-200 hover:bg-gray-50 transition" id="shelf-row-{{ $shelf->id }}">
                            <td class="py-4 px-6 font-medium text-gray-800">{{ $shelf->shelf_number }}</td>
                            <td class="py-4 px-6">{{ $shelf->description ?? 'N/A' }}</td>
                            <td class="py-4 px-6">
                                <span class="bg-amber-100 text-amber-800 font-semibold py-1 px-3 rounded-full text-xs">
                                    {{ $bedCampuses[$shelf->campus] ?? ($shelf->campus ?? 'N/A') }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-4 px-6 text-center text-gray-400 italic">No BED shelves found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <select name="campus" id="add_campus"
                                    ...
                                    @foreach ($bedCampuses as $value => $label)
